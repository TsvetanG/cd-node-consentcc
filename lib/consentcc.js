...
var Chaincode = class {
	...
	async query(stub, args) {
		...
		if (args.length > 2 || args.length === 0) {
			return shim.error('Expecting client ID and/or MSP id as input');
		}
		let clientID = args[0];
		...
		logger.info("Query consent for MSP id: " + mspId);
		
		let jsonResp;
	 	
		let iterator;
		if (mspId) {
			iterator = await stub.getStateByPartialCompositeKey("clid~mspid", [ clientID, mspId ] );
			logger.info("Query consent for client and MSP id");
		} else {
			iterator = await stub.getStateByPartialCompositeKey("clid~mspid", [ clientID ] );
			logger.info("Query consent for client");
		}
